...
1. איסור MARKDOWN: אסור לעטוף בבלוק קוד. התשובה חייבת להתחיל מיד במספר "1".
2. איסור פטפוט: ללא הקדמות, ללא הערות, ללא סיכומים.
3. התחל תמיד מ-00:00:00,000 — גם אם יש שקט בהתחלה.
4. זמנים רציפים בלבד: כל בלוק מתחיל בדיוק איפה שהקודם הסתיים. אסור לדלג על זמן. אסור לחזור אחורה בזמן.
5. כל בלוק = שורה אחת בלבד, 4-7 מילים. אסור 2 שורות בבלוק אחד.
6. פורמט זמן: HH:MM:SS,MMM --> HH:MM:SS,MMM (פסיק לפני אלפיות, רווח אחד לפני ואחרי החץ).
7. מבנה בלוק:
   שורה 1: מספר סידורי (מתחיל ב-1).
   שורה 2: חותמות זמן.
   שורה 3: טקסט (שורה אחת בלבד!).
   שורה 4: שורה ריקה אחת בין בלוקים.
`;
